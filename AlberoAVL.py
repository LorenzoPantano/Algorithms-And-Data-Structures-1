...
class AVLTree(BinarySearchTree):
    ...
    def root(self):              # Nuovo metodo per ritornare la radice di un albero AVL
        root = self.tree.root
        if root == None:
            return None
        else:
    ...
    def cambiaValore(self, key, value):  # Nuovo metodo per cambiare il valore di un nodo
        node = self.searchNode(key)
        node.info[self.VALUE_INDEX] = value
